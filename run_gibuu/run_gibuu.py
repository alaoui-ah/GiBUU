...
# ex: ./run_gibuu.py -mode test -ebeam 11.0 -seed 0 -targ C -tgzpos -3.0 -tgzlen 0.5 -tgrad 1.0 -kt 0.64 -expt clas11 -run1 1 -run2 1 -oudir /work/clas12/ahmed/
##
...
modes = ["test", "farm"]
experiments = ["hermes","clas6", "clas11", "none"]
expID = {experiments[0]:1, experiments[1]:5, experiments[2]:4, experiments[3]:17}
...
if expt not in experiments:
  print(f"Bad experiment argument. Possible values are: {experiments}\n")
  exit(0)
...
scriptDir = f"{softDir}/GiBUU/run_{gene}/".strip()
...
logDir = f"{gene}/{gibVer}/{eBeam}GeV/{targ}/kt_{kt}/".strip()
...
for ir in range(int(run1), int(run2) + 1, 1):
	...
	optFile  = f"{gene}_{btr}.opt".strip()
	simFile  = f"{gene}_{btr}.py".strip()
	logFile  = f"{gene}_{btr}.log".strip()
	seedFile = f"{gene}_{btr}.seed".strip()
	rootFile = f"{gene}_{btr}.root".strip()
	lundFile = f"{gene}_{btr}.lund".strip()
	...
	fin0.write("\n")
	fin0.write(f"Input Argument seed: {seed}, {gene} seed: {mySeed}\n")
	fin0.write("\n")
	fin0.close()
  ...
  #option file
	...
	myfuncs.sedCmd(optFile, "iExperiment=", f"iExperiment={expID[expt]}")
  ...
  #simulation  file
	...
	if mode == "test":
		...
		errFile = f"{farmOuDir}{gene}_{btr}.err".strip()
		outFile = f"{farmOuDir}{gene}_{btr}.out".strip()
		jobFile = f"{gene}_{btr}.sh".strip()

		#email = 
		myTime = "72:00:00"
		diskSpace = "5000M"
		memUsage = "1000M"
		jobName = f"{gene}_{btr}".strip()
		osName = "general"
		queue = "production"
		proj = "clas12"
		machine = "JLAB"
